...
Window::Window(Camera *const camera) {
    ...
    width = static_cast<float>(mode->width)/2.0f;
    height = static_cast<float>(mode->height)/2.0f;
    ...
    window = glfwCreateWindow(static_cast<int>(width), static_cast<int>(height), "Forest", nullptr, nullptr);
    if (window == nullptr) {
