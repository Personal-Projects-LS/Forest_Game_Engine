...
Terrain::Terrain(TerrainTextureMap textureMap, TerrainMesh *mesh, int gridX, int gridZ) noexcept {
    float x = static_cast<float>(gridX) * TerrainMesh::SIZE;
    float z = static_cast<float>(gridZ) * TerrainMesh::SIZE;
    terrainMesh = mesh;
    terrainTextureMap = std::move(textureMap);
    position = glm::vec3(x, 0, z);
}

void Terrain::render(const Camera& camera, const Shader& shader, glm::vec3 lightPos, glm::vec3 lightColor) const noexcept {
    shader.use();
    terrainMesh->bindVAO();
